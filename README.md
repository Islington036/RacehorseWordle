...
初期データは JRA 公式の過去 G1 成績ページを主軸に自動生成し、地方 GI/JpnI は Wikipedia の歴代優勝馬表と競走馬 infobox を補助ソースとして照合します。公開用の出題 JSON は `data/questions.json` に統合し、直接起動用の `data/questions.embedded.js` には同じ内容を埋め込んでいます。生成時の監査情報は公開データに含めず、ローカルキャッシュ `.cache/sources/race-wins.audit.json` に保存します。
